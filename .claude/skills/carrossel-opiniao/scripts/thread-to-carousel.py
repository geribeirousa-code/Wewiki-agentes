...
THEMES = {
    ...
    "bowlgreen": {
        "bg": "#F2EFE4",        # Creme
        "text": "#24573A",      # Verde Bowl
        "handle": "#C9A24B",    # Dourado
        "verified": "#E8684A",   # Coral
        "divider": "#E4EBDD",   # Sálvia
        "placeholder": "#E4EBDD",
    },
    # Clean Touch Cabinets — paleta de CLEAN TOUCH CABINETS/CONTEXTO-MARCA.md §Visual.
    # Cada cor usada no papel que a bíblia da marca atribui a ela.
    "cleantouch": {
        "bg": "#F0DDD3",        # Terracota 1 — "fundos"
        "text": "#36646E",      # Teal escuro — "títulos, texto forte"
        "handle": "#8A4A36",    # Terracota 5 — "texto sobre terracota clara"
        "verified": "#A9634A",   # Terracota 4 — "botão principal no fundo claro, links"
        "divider": "#D9A891",   # Terracota 2 — "bordas"
        "placeholder": "#8FBEC1",  # Teal claro — "marca, fundos, tags"
    },
}
...
def _variable_font(path, size, weight_name):
    """Load a variable font at a named weight (e.g. 'Bold', 'Regular').

    Without selecting the axis, a variable font renders at its default
    instance, so a 'bold' name would come out the same weight as the body.
    Falls back to the default instance if the build lacks named variations.
    """
...
def load_fonts(theme_name="light"):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    fonts_dir = os.path.join(os.path.dirname(script_dir), "fonts")
    
    # Custom Bowl Green fonts
    baloo2_path = os.path.join(fonts_dir, "Baloo2-VariableFont.ttf")
    nunito_path = os.path.join(fonts_dir, "Nunito-VariableFont.ttf")
    
    # Clean Touch — Inter. Fonte variável: o peso Bold é extraído por eixo,
    # senão o nome do perfil sairia com a mesma espessura do corpo.
    inter_path = os.path.join(fonts_dir, "InterVariable.ttf")

    if theme_name == "cleantouch" and os.path.exists(inter_path):
        return {
            "name_bold": _variable_font(inter_path, NAME_SIZE, "Bold"),
            "handle": _variable_font(inter_path, HANDLE_SIZE, "Regular"),
            "tweet": _variable_font(inter_path, TWEET_SIZE, "Regular"),
        }

    if theme_name == "bowlgreen" and os.path.exists(baloo2_path) and os.path.exists(nunito_path):
        bold_candidates = [baloo2_path]
        regular_candidates = [nunito_path]
    else:
        bold_candidates = [
